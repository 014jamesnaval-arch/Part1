...
class AiScheduleService extends ChangeNotifier{
    ScheduleAnaylsis? _currentAnalysis;
    bool _isLoading = false;
    String? _errorMessage;

    final String _apiKey = '';

    ScheduleAnaylsis? get currentAnalysis => _currentAnalysis;
    ...
    ScheduleAnaylsis _parseResponse(String fullText){
        ...
        for (var section in sections) {
            ...
            else if (section.startsWith('Ranked Tasks')) conflicts = section.replaceFirst('Ranked Tasks', '').trim();
            else if (section.startsWith('Recommended Schedule')) conflicts = section.replaceFirst('Recommended Schedule', '').trim();
            else if (section.startsWith('Explanations')) conflicts = section.replaceFirst('Explanation', '').trim();
        }
        return ScheduleAnaylsis(conflicts: conflicts, rankedTasks : rankedTasks, recommendedSchedule : recommendedSchedule, explanation: explanation);
    }
    }
}